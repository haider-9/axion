...
import { Search, Handbag, UserRound, ChevronDown, Clock, Star, Trash2, Plus, Minus } from 'lucide-react';
...
const Header = () => {
  ...
  const navLinks = [
    { name: 'Home', href: '/' },
    { name: 'Products', href: '/product', hasDropdown: true, dropdownType: 'products' },
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-around  py-4 mx-auto bg-transparent">
      ...
      <div className="size-16 flex items-center">
        <Image src="/Logo.png" alt="Axion Lighting Solutions Logo" width={100} height={50} />
      </div>

      {/* Navbar with pill animation */}
      <div className="relative py-1 px-1 rounded-full shadow-lg backdrop-blur-md bg-gray-800/30">
          ...
          {navLinks.map((link) => {
            ...
            return (
              ...
              >
                ...
                <Link
                  href={link.href}
                  className={`relative z-10 rounded-full px-5 py-2 flex items-center gap-1 ${isTarget ? 'text-black' : 'text-white'
                    }`}
                >
                  {link.name}
                  {link.hasDropdown && (
                    <ChevronDown
                      size={16}
                      className={`transition-transform duration-200 ${(link.dropdownType === 'products' && showProductsDropdown) ||
      ...
      {/* Icons */}
      <div className="flex items-center bg-slate-700/50 backdrop-blur-xl rounded-full p-2 shadow-2xl border border-white/20">
            ...
            <DialogContent className="max-w-2xl" >
              ...
              <div className="space-y-6">
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 transform -translate-y-1/2" size={20} style={{ color: '#2CA6A4' }} />
                  <input
                    type="text"
                ...
                <div className="space-y-4">
                  <div>
                    <h3 className="flex items-center gap-2 font-medium mb-3" style={{ color: '#0C1E33' }}>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {popularProducts.map((product, index) => (
                        ...
                        >
                          <div className="font-medium" style={{ color: '#0C1E33' }}>{product.name}</div>
                          <div className="text-sm" style={{ color: '#2CA6A4' }}>{product.category}</div>
                        </button>
                      ))}
                    </div>
                  </div>
          ...
          <DropdownMenu>
            ...
            >
              ...
              <div className="space-y-3 max-h-64 overflow-y-auto">
                {cartItems.map((item) => (
                  <div key={item.id} className="flex items-center space-x-3 p-2 rounded-lg hover:bg-[#2CA6A4]/10">
                    <Image src={item.image} alt={item.name} width={40} height={40} className="rounded" />
                    <div className="flex-1">
                      <h4 className="font-medium text-sm" style={{ color: '#0C1E33' }}>{item.name}</h4>
                      <p className="text-xs" style={{ color: '#2CA6A4' }}>${item.price}</p>
                    </div>
                    <div className="flex items-center space-x-2">
                      <button className="w-6 h-6 rounded-full flex items-center justify-center" style={{ backgroundColor: '#E1B857' }}>
                        <Minus size={12} style={{ color: '#0C1E33' }} />
                      </button>
                      <span className="text-sm" style={{ color: '#0C1E33' }}>{item.quantity}</span>
                      <button className="w-6 h-6 rounded-full flex items-center justify-center" style={{ backgroundColor: '#E1B857' }}>
              ...
              <div className="space-y-3 pt-2">
                <div className="flex justify-between items-center">
                  <span className="font-semibold" style={{ color: '#0C1E33' }}>Total:</span>
