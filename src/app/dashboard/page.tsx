            ...
            <nav className="space-y-1 text-sm">
              {[
                ...
                <Link
                  key={item.label}
                  href={item.href}
                  className="block px-3 py-2 rounded-lg hover:bg-muted aria-[current=true]:bg-black/5"
                  aria-current={item.href === '/dashboard'}
                >
                  {item.label}
                </Link>
              ))}
              <button className="w-full px-3 py-2 text-left rounded-lg hover:bg-muted">
                Logout
              </button>
            </nav>
          </div>
            ...
            <CardContent>
              <div className="h-40 rounded-md border flex items-end gap-1 p-3">
                {Array.from({ length: 24 }).map((_, i) => (
                  <div
                    key={i}
                    className="flex-1 bg-[#2CA6A4]/20"
                    style={{ height: `${10 + ((i * 3) % 90)}%` }}
                  />
                ))}
              </div>
            </CardContent>
                    ...
                    {[1, 2, 3, 4, 5].map((i) => (
                      <TableRow key={i}>
                        ...
                        <TableCell className="text-emerald-600">
                          Rs.{(59999 - i * 2000).toLocaleString()}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                ...
                <TableBody>
                  {[
                    ...
                    <TableRow key={idx}>
                      ...
                      <TableCell>
                        <span className={`px-2 py-1 text-xs text-white rounded ${p.color}`}>
                          {p.status}
                        </span>
                      </TableCell>
                      <TableCell>
                        <button className="px-3 py-1 text-sm rounded bg-muted hover:bg-black/5">
                          Restock
                        </button>
                      </TableCell>
                    </TableRow>
